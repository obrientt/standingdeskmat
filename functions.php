...
if( !function_exists( 'tz_enqueue_scripts' ) ) {
    function tz_enqueue_scripts() {
		...
		wp_register_script('tz_custom', get_template_directory_uri() . '/js/jquery.custom.js', array('jquery', 'jquery-ui-custom', 'slides', 'isotope', 'superfish'), '1.0', TRUE);
...
if( !function_exists( 'tz_wp_login_url' ) ) {
    function tz_wp_login_url() { echo home_url(); }
    
    add_filter('login_headerurl', 'tz_wp_login_url');
}

if( !function_exists( 'tz_wp_login_title' ) ) {
    function tz_wp_login_title() { echo get_option('blogname'); }
